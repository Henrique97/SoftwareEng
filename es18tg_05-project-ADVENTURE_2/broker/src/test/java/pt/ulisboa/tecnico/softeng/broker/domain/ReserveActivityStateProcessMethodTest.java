...
public class ReserveActivityStateProcessMethodTest {
	...
	private static final String IBANb = "BK0198765431";
	...
	@Before
	public void setUp() {
		this.adventure = new Adventure(this.broker, begin, end, new Client(IBAN, AGE, BUYERNIF), AMOUNT);
	...
	public void successNoBookRoom(@Mocked final ActivityInterface activityInterface) {
		Adventure sameDayAdventure = new Adventure(this.broker, begin, begin, new Client(IBAN, AGE, BUYERNIF), AMOUNT);
		sameDayAdventure.setState(State.RESERVE_ACTIVITY);

		new Expectations() {
			{
				ActivityInterface.reserveActivity(begin, begin, AGE,broker.getBuyerNIF(), broker.getIBAN());
		...
		Assert.assertEquals(State.RENT_STATE, sameDayAdventure.getState());
	}
	...
	public void successBookRoom(@Mocked final ActivityInterface activityInterface) {
		new Expectations() {
			{
				ActivityInterface.reserveActivity(begin, end, AGE,broker.getBuyerNIF(), broker.getIBAN());
				this.result = ACTIVITY_CONFIRMATION;
			}
		};
