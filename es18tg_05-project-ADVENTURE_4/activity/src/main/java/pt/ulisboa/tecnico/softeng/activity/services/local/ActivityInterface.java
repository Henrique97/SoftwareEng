...
public class ActivityInterface {
	...
	@Atomic(mode = TxMode.WRITE)
	public static String reserveActivity(String codeProvider, String codeActivity, String codeOffer, ActivityReservationData reservation) {
		
		Activity activity = ActivityInterface.getActivityByCode(codeProvider, codeActivity);
		ActivityProvider provider = ActivityInterface.getProviderByCode(codeProvider);
		if (provider!=null && activity !=null) {
			for (ActivityOffer offer: activity.getActivityOfferSet()) {
				try {
					if(Integer.toString(offer.getCode()).equals(codeOffer))
						return new Booking(provider, offer, reservation.getNif(), reservation.getIban()).getReference();
				} catch (Exception e) {
						throw new ActivityException();
				}
			}
